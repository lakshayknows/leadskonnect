...
async function pollOnce() {
  ...
  if (!action) return schedule(75); // idle — check again shortly
...
chrome.runtime.onInstalled.addListener(() => schedule(5));
chrome.runtime.onStartup.addListener(() => schedule(5));
chrome.storage.onChanged.addListener((ch) => { if (ch.enabled && ch.enabled.newValue) schedule(3); });
